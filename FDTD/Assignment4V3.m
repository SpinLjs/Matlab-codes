...
for t=dt:dt:5*dt
    ...
    % PML region
    Hx(Reg_1y,1:end-1)=(2*u0-dt.*sigma_my(Reg_1y,1:end-1))./(2*u0+dt.*sigma_my(Reg_1y,1:end-1))...
        .*Hx(Reg_1y,1:end-1)-2*dt/dy./(2*u0+dt.*sigma_my(Reg_1y,1:end-1))...
        .*(Ez(Reg_1y+1,1:end-1)-Ez(Reg_1y,1:end-1));
    Hx(Reg_3y,1:end-1)=(2*u0-dt.*sigma_my(Reg_3y,1:end-1))./(2*u0+dt.*sigma_my(Reg_3y,1:end-1))...
        .*Hx(Reg_3y,1:end-1)-2*dt/dy./(2*u0+dt.*sigma_my(Reg_3y,1:end-1))...
    ...
    Hy(Reg_1y,1:end-1)=(2*u0-dt.*sigma_mx(Reg_1y,1:end-1))./(2*u0+dt.*sigma_mx(Reg_1y,1:end-1))...
        .*Hy(Reg_1y,1:end-1)+2*dt/dx./(2*u0+dt.*sigma_mx(Reg_1y,1:end-1))...
        .*(Ez(Reg_1y,2:end)-Ez(Reg_1y,1:end-1));
    Hy(Reg_3y,1:end-1)=(2*u0-dt.*sigma_mx(Reg_3y,1:end-1))./(2*u0+dt.*sigma_mx(Reg_3y,1:end-1))...
        .*Hy(Reg_3y,1:end-1)+2*dt/dx./(2*u0+dt.*sigma_mx(Reg_3y,1:end-1))...
        .*(Ez(Reg_3y,2:end)-Ez(Reg_3y,1:end-1));
    Hy(Reg_2y,Reg_1x)=(2*u0-dt.*sigma_mx(Reg_2y,Reg_1x))./(2*u0+dt.*sigma_mx(Reg_2y,Reg_1x))...
        .*Hy(Reg_2y,Reg_1x)+2*dt/dx./(2*u0+dt.*sigma_mx(Reg_2y,Reg_1x))...
        .*(Ez(Reg_2y,Reg_1x+1)-Ez(Reg_2y,Reg_1x));
    Hy(Reg_2y,Reg_3x)=(2*u0-dt.*sigma_mx(Reg_2y,Reg_3x))./(2*u0+dt.*sigma_mx(Reg_2y,Reg_3x))...
        .*Hy(Reg_2y,Reg_3x)+2*dt/dx./(2*u0+dt.*sigma_mx(Reg_2y,Reg_3x))...
    ...
    Ezx(Reg_e3y,2:end-1) = (2*e0-dt.*sigma_ex(Reg_e3y,2:end-1))./(2*e0+dt.*sigma_ex(Reg_e3y,2:end-1))...
        .*Ezx(Reg_e3y,2:end-1) + 2*dt/dx./((2*e0+dt.*sigma_ex(Reg_e3y,2:end-1)))...
        .*(Hy(Reg_e3y,2:end-1)-Hy(Reg_e3y,1:end-2));
    Ezy(Reg_e3y,2:end-1) = (2*e0-dt.*sigma_ey(Reg_e3y,2:end-1))./(2*e0+dt.*sigma_ey(Reg_e3y,2:end-1))...
        .*Ezy(Reg_e3y,2:end-1) - 2*dt/dy./((2*e0+dt.*sigma_ey(Reg_e3y,2:end-1)))...
        .*(Hx(Reg_e3y,2:end-1)-Hx(Reg_e3y-1,2:end-1));
    Ez(Reg_e3y,2:end-1)=Ezx(Reg_e3y,2:end-1)+Ezy(Reg_e3y,2:end-1);
