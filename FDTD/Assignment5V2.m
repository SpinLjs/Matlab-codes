...
h=0.3e-6;   % /m 
WR_l=1.6e-6;    % width of left waveguide   
WR_r=1.7e-6;    % width of right waveguide
S= 1e-6;
...
m_y = Ly/2;
% p=0.43;     % p,q adjust the x,y direction variance
% q=0.923;
...
%%-------------------------------------------------------------------------------------------------------
% update iteration
% function tridiag() to fast compute tri-diagonal matrix equation 
figure();
for n=1:length(z)-1
...
E_show2=reshape(E(:,:,450),length(y),length(x));
figure(3)
mesh(x,y,abs(E_show2));
xlabel('x axis / m');
ylabel('y axis / m');
zlabel('E_z magnitude / V/m');
colorbar();
